...
class Functions(private val gui: MainView): Closeable {
    ...
    fun generateUniqueFileName(originalFile: File, targetDirectory: File): File {
        val baseName = originalFile.nameWithoutExtension
        val extension = originalFile.extension
        var counter = 1
        var candidateFile = File(targetDirectory, originalFile.name)

        // 循环检查文件是否存在，直到找到未被占用的文件名
        while (candidateFile.exists()) {
            val newFileName = "$baseName${if (counter > 1) "_$counter" else ""}.$extension"
            candidateFile = File(targetDirectory, newFileName)
            counter++
        }
    ...
    fun saveGroupedPainting() {
        val targetDir = File(configurations.path)

        if (continuation.faceComponent.isImported) {
            val baseName = "${continuation.taskName}_exp.png"
            val uniqueFile = generateUniqueFileName(File(baseName), targetDir)

            getFaceGroupedPainting(false)
                .flipY()
                .apply()
                .savePng(uniqueFile, configurations.pngCompressionLevel)
        } else {
            val baseName = "${continuation.taskName}_group.png"
            val uniqueFile = generateUniqueFileName(File(baseName), targetDir)

            continuation.groupedPainting
                .flipY()
                .apply()
                .savePng(uniqueFile, configurations.pngCompressionLevel)
        }
    ...
    fun saveAllFacePainting() {
        ...
        for ((i, group) in groupList.withIndex()) {
            ...
            val fileName = generateUniqueFileName(File(baseName), File(configurations.path))
